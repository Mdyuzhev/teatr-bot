...
| T003 | ✅ DONE | RSS-коллектор + обогащение дайджеста + /news |
| T004 | ✅ DONE | Тесты (63 unit) + GitHub Actions CI/CD + production деплой |
...
6. Переместить задачу в `Tasks/done/`, обновить этот файл
...
**Деплой (через MCP):**
1. `git push` в GitHub
2. `git pull` на сервере через `mcp__homelab__run_shell_command`
3. Перезапуск бота через MCP (фоновая команда)
4. Проверка статуса каждые 30 секунд
